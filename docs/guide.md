...
[Get started](#get-started) · [Configuration](#configuration) · [Context compaction](#context-compaction) · [Development](#development) · [Roadmap](roadmap.md)
...
Install a tagged release for normal use. Provider behavior depends on the endpoint and model.
...
You can also use `go install github.com/muratmirgun/owncode@latest` to install the latest tagged release.
...
Use the [compatible provider example](theykk.example.json) as a template.
...
The [example configuration](theykk.example.json) shows sampling options, image support, and interleaved reasoning.
...
The agent receives LSP diagnostics and can request definitions, references, and symbols. Rename and code actions are not exposed.
...
See [Witch orchestration](witch.md) for the role matrix and boundaries.
...
The [first-run report](first-run.md) records the initial fork state, not the current feature set.
The [roadmap](roadmap.md) records the implementation scope, reference sources, and remaining work.
